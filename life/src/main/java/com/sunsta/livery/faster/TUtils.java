...
public class TUtils {
    ...
    public static void captureBySafely(TContextWrap contextWrap, TIntentWap intentWap) throws TException {
        List result = contextWrap.getActivity().getPackageManager().queryIntentActivities(intentWap.getIntent(), PackageManager.MATCH_ALL);
        if (result.isEmpty()) {
            ToastUtils.s(contextWrap.getActivity(), StringUtils.getString(R.string.text_no_camera));
    ...
    public static void cropWithOtherAppBySafely(TContextWrap contextWrap, Uri imageUri, Uri outPutUri, CropOptions options) {
        Intent nativeCropIntent = FasteIntent.getCropIntentWithOtherApp(imageUri, outPutUri, options);
...
//                e.printStackTrace();
//            }
            startActivityForResult(contextWrap, new TIntentWap(FasteIntent.getCropIntentWithOtherApp(imageUri, outPutUri, options), AnConstants.CAPTURE.RC_CROP));
        }
    }
...
//        }
    ...
    public static ProgressDialog showProgressDialog(final Activity activity,
        ...
        String title = activity.getResources().getString(R.string.text_compress_now);
