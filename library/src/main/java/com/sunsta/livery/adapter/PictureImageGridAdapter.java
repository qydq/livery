package com.sunsta.livery.adapter;
...
import com.sunsta.livery.tools.VoiceUtils;
...
public class PictureImageGridAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == PictureConfig.TYPE_CAMERA) {
            View view = LayoutInflater.from(context).inflate(R.layout.an_item_camera, parent, false);
            return new CameraViewHolder(view);
        } else {
            View view = LayoutInflater.from(context).inflate(R.layout.an_image_grid_item, parent, false);
        ...
        if (getItemViewType(position) == PictureConfig.TYPE_CAMERA) {
            ...
            if (eqVideo || eqAudio) {
                ...
                contentHolder.tvDuration.setCompoundDrawablesRelativeWithIntrinsicBounds
                        (eqVideo ? R.mipmap.ic_white_video : R.mipmap.ic_white_audio,
                                0, 0, 0);
            } else {
        ...
        if (isChecked) {
            for (int i = 0; i < count; i++) {
                ...
                if (media.getPath().equals(image.getPath())
                    ...
                    subSelectPosition();
                    ViewUtils.getInstance().disZoom(contentHolder.ivPicture, config.zoomAnim);
                    break;
                }
            }
        } else {
            ...
            ViewUtils.getInstance().zoom(contentHolder.ivPicture, config.zoomAnim);
